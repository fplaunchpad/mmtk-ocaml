...
gcc -c tests/ocaml-5/stubs.c -I"$OCAML_INC" -Immtk-ocaml-5/include -o tests/ocaml-5/stubs.o
ocamlopt tests/ocaml-5/stubs.o tests/ocaml-5/test_nogc.ml \
    -ccopt "-L$LIB_DIR -Wl,-rpath,$LIB_DIR" \
    -cclib -lmmtk_ocaml_5 -cclib -lpthread \
    -o tests/ocaml-5/test_nogc_5
./tests/ocaml-5/test_nogc_5
```
...
| File | Role |
|---|---|
| `tests/ocaml-5/test_nogc_c.c` | Pure-C end-to-end test; no OCaml runtime |
| `tests/ocaml-5/stubs.c` | `CAMLprim` C stub that bridges OCaml and MMTk; lazily inits MMTk |
| `tests/ocaml-5/test_nogc.ml` | OCaml 5.x test program; calls `caml_mmtk5_alloc_print` via external |
