...
`tests/ocaml-4/test_nogc.c` calls the MMTk C API directly:
...
gcc tests/ocaml-4/test_nogc.c \
    target/debug/libmmtk_ocaml_4.a \
    -Immtk-ocaml-4/include -lpthread -ldl -lm \
    -o tests/ocaml-4/test_nogc_4
./tests/ocaml-4/test_nogc_4
```
...
`tests/ocaml-4/stubs_4.c` exposes a single OCaml primitive
...
`tests/ocaml-4/test_nogc.ml` declares the external binding, prints a header, calls
...
gcc -c tests/ocaml-4/stubs_4.c -I"$OCAML_INC" -Immtk-ocaml-4/include -o tests/ocaml-4/stubs_4.o
ocamlopt tests/ocaml-4/stubs_4.o tests/ocaml-4/test_nogc.ml \
    -ccopt "-L$LIB_DIR -Wl,-rpath,$LIB_DIR" \
    -cclib -lmmtk_ocaml_4 -cclib -lpthread \
    -o tests/ocaml-4/test_nogc_4_ml
./tests/ocaml-4/test_nogc_4_ml
```
...
| File | Role |
|------|------|
| `tests/ocaml-4/test_nogc.c` | Pure C harness — exercises the MMTk C API with no OCaml runtime |
| `tests/ocaml-4/stubs_4.c` | C stub — bridges the OCaml 4.14 calling convention to the MMTk API |
| `tests/ocaml-4/test_nogc.ml` | OCaml 4.14 driver — calls the stub for `wosize = 1..5` |
